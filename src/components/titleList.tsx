...
export const TitleList = (props: TitlesProps) => {
  ...
  return (
    <div className="grid grid-cols-3 bg-blue-400 gap-2 m-4 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 2xl:grid-cols-8">
      <AddTitle uuid={props.uuid} getTitleList={props.getTitleList} />
      {filteredTitle.map((title, index) => {
        return (
          <Link key={index} href={`/title?id=${title.id}`} passHref>
